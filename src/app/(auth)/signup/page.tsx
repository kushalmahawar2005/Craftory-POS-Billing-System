  ...
  if (step === 'otp') {
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="mb-6 p-3 bg-primary-light border border-primary/20 text-text-primary text-sm rounded-lg flex items-center justify-between shadow-sm">
          <span><strong>Demo Mode:</strong> Any 6-digit code works</span>
          <button 
            onClick={() => setOtp(['1','2','3','4','5','6'])} 
            className="px-3 py-1.5 bg-white text-primary rounded-md text-xs font-bold shadow-sm hover:bg-gray-50 border border-primary/10 transition-colors"
          >
            Auto-fill
          </button>
        </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <button
        ...
        disabled={loading || loadingGoogle}
        className="w-full h-11 flex items-center justify-center gap-3 bg-white border border-border rounded-xl hover:bg-gray-50 hover:border-gray-300 disabled:opacity-50 disabled:cursor-not-allowed transition-all text-sm font-medium text-text-primary relative overflow-hidden"
      >
        {loadingGoogle ? (
